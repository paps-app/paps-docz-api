...
- Création de tache de ramassage (`createPickup`)
- Création de tache de livraison (`createDelivery`)
- Création de tâches multiples (`createMultipleTasks`)

> Toute création de tache doit être faite avec un argument de type query `apiKey` que vous pouvez
...
Cette méthode est utilisée pour créer une tâche de livraison. Une tâche de livraison désigne une course où le colis doit être déposé. Elle est donc souvent précédée et liée à une tache de ramassage. Le tableau ci-dessous récapitule les paramètres à accompagner avec la requête de création de tâches de livraison:
